...
public class Solution {
  public List<Interval> insert(List<Interval> intervals, Interval newInterval) {
    ...
    for (Interval interval : intervals) {
      if (newInterval.start > interval.start
          && newInterval.start < interval.end) { // for e.g I(2,4) NI (3,5)
        minInSet = interval.start;
        maxInSet = Math.max(maxInSet, newInterval.end);
      }
      if (newInterval.end > interval.start
          && newInterval.end < interval.end) { // for e.g I(8,10) NI(4,9)
  ...
  public static void main(String[] args) {
    List<Interval> insert =
        new Solution().insert(List.of(new Interval(1, 3), new Interval(6, 9)), new Interval(2, 5));
