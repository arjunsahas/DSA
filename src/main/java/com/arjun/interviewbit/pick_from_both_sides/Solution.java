 ...
 *
 * <p>This is a sliding window problem.
 */
public class Solution {
  ...
  public int solve2(List<Integer> A, int B) {
    if (B > A.size()) return -1;
    if (A.size() == 1) return A.get(0);
    if (A.size() == 2) return A.get(0) > A.get(1) ? A.get(0) : A.get(1);
    ...
    for (int i = 0; i < B + 1; i++) {
      int sum = 0;
      if (i - 1 >= 0) {
        int startPos = startWindow < 0 ? startWindow + A.size() : startWindow;
        int endPos = endWindow < 0 ? endWindow - 1 + A.size() : endWindow - 1;
        int prevSum = localSums.get(i - 1);
        int currentSum =
            prevSum + A.get(startPos) - A.get(endPos + 1 < A.size() ? endPos + 1 : endPos);
        localSums.add(currentSum);
      } else {
        for (int j = startWindow; j < endWindow; j++) {
          int pos = j < 0 ? j + A.size() : j;
          sum = sum + A.get(pos);
  ...
  public static void main(String[] args) {
    ...
    solve = new Solution().solve2(new ArrayList<>(List.of(5, -2, 3, 1, 2)), 3);
    System.out.println(solve);

    solve = new Solution().solve2(new ArrayList<>(List.of(5, -2, 3, 1, 2)), 4);
    System.out.println(solve);

    solve = new Solution().solve2(new ArrayList<>(List.of(5, -2, 3, 1, 2)), 5);
    System.out.println(solve);

    solve = new Solution().solve2(new ArrayList<>(List.of(1, 2)), 1);
    System.out.println(solve);
    ...
    solve = new Solution().solve2(Arrays.asList(array), 32);
    System.out.println(solve);
  }
}
