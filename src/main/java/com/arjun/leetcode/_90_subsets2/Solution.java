...
class Solution {
    ...
    private void subsets(List<Integer> numsList, List<List<Integer>> subsets, List<Integer> workingList, int start) {
    ...
    private boolean isaBoolean(List<Integer> numsList, int start, int end) {
        return start <= end && start <= numsList.size() && end <= numsList.size() && start >= 0 && end >= 0;
    }
    ...
    private void addIfUnique(List<Integer> incomingSubset, List<List<Integer>> existingSubsets) {
        ...
        if (uniqueForEntireSet)
            existingSubsets.add(incomingSubset);
    }
