 ...
 *
 * <p>The base condition is that the substring of s1 and s2 needs to match with the s3 string.
 *
 * <p>The intuition is that you consider the substring of s1 or s2. The consideration is based on
 * condition whether there is a partiol match to the s3 string.
 */
public class Solution {
  ...
  private boolean checkInterleave(String s1, String s2, String s3, int i, int j, String result) {
    ...
    if (s3.startsWith(result + c + c1) || s3.startsWith(result + c1 + c)) {
      ...
      //            j++;
      ret |=
          checkInterleave(s1, s2, s3, ++i, ++j, result + c + c1)
              || checkInterleave(s1, s2, s3, ++i, ++j, result + c1 + c);
  ...
  private boolean checkInterleave1(String s1, String s2, String s3, int i, int j, int k) {
    ...
    if (c == c2 && c1 == c2) {
      System.out.print(c2);
      ret =
          checkInterleave1(s1, s2, s3, i + 1, j, k + 1)
              || checkInterleave1(s1, s2, s3, i, j + 1, k + 1);
    } else if (c2 == c1) {
      System.out.print(c2);
