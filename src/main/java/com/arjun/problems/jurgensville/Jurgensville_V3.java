...
public class Jurgensville_V3 {
    ...
    public static void main(String[] args) {
        if (args.length <= 1) {
            System.err.println("No inputs provided. Please provide an input .csv file followed by required restaurant items");
            return;
        }
    ...
    private static Map<String, List<MenuRecord>> buildRestaurantMap(Map<String, List<MenuRecord>> menuMap, String line) {
                ...
                if (menuRecord.getItems().contains(orderItem)) {
                    ...
                    if (result1 != null) {
                        if (!isDuplicates(menuRecord.getItems(), result1.getItems())) {
                            Result result = new Result(restaurantId, menuRecord.getPrice().add(result1.getPrice()), menuRecord.getItems());
