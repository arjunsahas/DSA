...
public class TreeMap<K, V> extends AbstractMap<K, V>
   ...
   * (except through the iterator's own {@code remove} operation), the results of the iteration are
   * undefined. The set supports element removal, which removes the corresponding mapping from the
   * map, via the {@code Iterator.remove}, {@code Set.remove}, {@code removeAll}, {@code retainAll},
   ...
   * in progress (except through the iterator's own {@code remove} operation, or through the {@code
   * setValue} operation on a map entry returned by the iterator) the results of the iteration are
   * undefined. The set supports element removal, which removes the corresponding mapping from the
   * map, via the {@code Iterator.remove}, {@code Set.remove}, {@code removeAll}, {@code retainAll}
   * and {@code clear} operations. It does not support the {@code add} or {@code addAll} operations.
   */
  ...
  static final class KeySet<E> extends AbstractSet<E> implements NavigableSet<E> {
    ...
    public Iterator<E> iterator() {
      if (m instanceof TreeMap) return ((TreeMap<E, Object>) m).keyIterator();
      else return (Iterator<E>) (((NavigableSubMap) m).keyIterator());
    }

    public Iterator<E> descendingIterator() {
      if (m instanceof TreeMap) return ((TreeMap<E, Object>) m).descendingKeyIterator();
      else return (Iterator<E>) (((NavigableSubMap) m).descendingKeyIterator());
    }
  ...
  static final class AscendingSubMap<K, V> extends NavigableSubMap<K, V> {
    ...
    public NavigableMap<K, V> descendingMap() {
      ...
      return (mv != null)
          ? mv
          : (descendingMapView =
              new DescendingSubMap(m, fromStart, lo, loInclusive, toEnd, hi, hiInclusive));
    }
  ...
  static final class DescendingSubMap<K, V> extends NavigableSubMap<K, V> {
    ...
    public NavigableMap<K, V> descendingMap() {
      ...
      return (mv != null)
          ? mv
          : (descendingMapView =
              new AscendingSubMap(m, fromStart, lo, loInclusive, toEnd, hi, hiInclusive));
    }
   ...
   * TreeMap that did not support NavigableMap. It translates an old-version SubMap into a
   ...
   *
   * <p>Implementations of rebalancings during insertion and deletion are slightly different than
   * the CLR version. Rather than using dummy nilnodes, we use a set of accessors that deal properly
   ...
   *     represented by the TreeMap. The key-value mappings are emitted in key-order (as determined
   *     by the TreeMap's Comparator, or by the keys' natural ordering if the TreeMap has no
   *     Comparator).
   */
   ...
   *
   * <p>1) An iterator of Map.Entries. (it != null, defaultVal == null). 2) An iterator of keys. (it
   ...
   *
   * <p>It is assumed that the comparator of the TreeMap is already set prior to calling this
   * method.
   *
